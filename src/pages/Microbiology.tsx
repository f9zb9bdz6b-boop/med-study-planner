...
const Microbiology: React.FC = () => {
  ...
  const categories = {
    general: microbiologyTopics.filter(t => t.name.includes('General')),
    bacteriology: microbiologyTopics.filter(t => t.name.includes('Bacteriology')),
    virology: microbiologyTopics.filter(t => t.name.includes('Virology')),
    parasitology: microbiologyTopics.filter(t => t.name.includes('Parasitology')),
    mycology: microbiologyTopics.filter(t => t.name.includes('Mycology')),
  };

  const renderCategory = (title: string, topics: typeof microbiologyTopics) => {
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-xs text-gray-500 dark:text-gray-400">
                        {topic.revisions}/{topic.targetRevisions} revisions
                      </span>
                    </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 pb-20">
      <Header title="Microbiology" />
      
      <main className="pt-16 px-4 max-w-screen-xl mx-auto">
        <div className="mt-4">
          {renderCategory('General Microbiology', categories.general)}
          {renderCategory('Bacteriology', categories.bacteriology)}
          {renderCategory('Virology', categories.virology)}
          {renderCategory('Parasitology', categories.parasitology)}
          {renderCategory('Mycology', categories.mycology)}
        </div>
      </main>
            ...
            <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 mb-4">
              <p className="text-sm text-gray-600 dark:text-gray-400">
                {showAIModal.action === 'explain' && 
                  'AI will provide detailed information about virulence factors, clinical features, and diagnosis.'}
                {showAIModal.action === 'mcqs' && 
                  'AI will generate MCQs for rapid revision and testing.'}
